...
function gerarLuminosidade() {
    if (modoManual)
        return luminosidadeManual;
    return Math.round(CONFIG.LUZ_MIN + Math.random() * (CONFIG.LUZ_MAX - CONFIG.LUZ_MIN));
}
function getTempStatus(v) {
    if (v < CONFIG.TEMP_OK_MAX)
        return 'OK';
    if (v <= CONFIG.TEMP_ALERTA_MAX)
        return 'ALERTA';
    return 'CRITICO';
}
function getLuzStatus(v) {
    if (v >= CONFIG.LUZ_OK_MIN && v <= CONFIG.LUZ_OK_MAX)
        return 'OK';
    if ((v >= CONFIG.LUZ_ALERTA_MIN && v < CONFIG.LUZ_OK_MIN) ||
        (v > CONFIG.LUZ_OK_MAX && v <= CONFIG.LUZ_ALERTA_MAX))
        return 'ALERTA';
    return 'CRITICO';
}
function consolidatePanel(tempStatus, luzStatus) {
    if (tempStatus === 'CRITICO' || luzStatus === 'CRITICO')
        return 'VERMELHO';
    if (tempStatus === 'ALERTA' || luzStatus === 'ALERTA')
        return 'AMARELO';
    return 'VERDE';
}
// ============ FUNÇÕES DE ENVIO ============
